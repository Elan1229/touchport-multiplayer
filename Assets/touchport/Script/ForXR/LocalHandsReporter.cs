...
public class LocalHandsReporter : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (headDevice.isValid && headDevice.TryGetFeatureValue(CommonUsages.devicePosition, out Vector3 headLocal))
        {
            var headWorld = xrOrigin != null ? xrOrigin.transform.TransformPoint(headLocal) : headLocal;
